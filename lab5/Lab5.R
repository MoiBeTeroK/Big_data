...
#5.2
# 1.	Добавить найденные в результате кластерного анализа классы, как вектор-столбец в предыдущий DATASET, разделить его на две части (обучающую и тестовую), обучить на обучающей выборке классификатор, а затем применить классификатор к тестовым данным.
# Для 3 кластеров
...
set.seed(123)
ind <- sample(2, nrow(data_2_new), replace = TRUE, prob = c(0.7, 0.3))  # 70% для тренировки и 30% для теста

trainData <- data_2_new[ind == 1, ]  # Обучающие данные
testData <- data_2_new[ind == 2, ]   # Тестовые данные

# 2. Решить задачу с помощью наивного Байесовского классификатора
library(klaR)
naive_model <- NaiveBayes(groups ~ ., data = trainData)

# Таблицы вероятностей для каждого критерия
naive_model$tables

summary(trainData)

par(mfrow = c(2, 3)) 
plot(naive_model, lwd = 2)
